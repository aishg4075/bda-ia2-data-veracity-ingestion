...
from ingestion_trust.core import (  # noqa: E402
    WinnerStyleConfig,
    append_audit_batch,
    build_run_metadata,
    detect_dataset_mode,
...
def main() -> None:
    ...
    for p in [metrics_dir, audit_dir, provenance_dir, curated_dir, debug_dir, args.figures_dir, args.model_dir]:
    ...
    _save_validation_reports(validation_summary, metrics_dir)

    validation_df = pd.read_csv(metrics_dir / "validation_report.csv")
    plot_data_quality_summary(validation_df, args.figures_dir / "data_quality_violations.png")
    plot_dq_rule_percentage(validation_df, args.figures_dir / "top_dq_rules_percentage.png")

    curated_pd = curated_df.toPandas()
    prevalence_plot_path = args.figures_dir / "attribution_prevalence.png"
    plot_fraud_prevalence(curated_pd, prevalence_plot_path, target_col=args.target_col)
    _ensure_legacy_compat_copy(
        prevalence_plot_path,
        args.figures_dir / "fraud_prevalence.png",
        compatibility_copy_map,
    )
    ...
    for track in tracks:
        track_metrics_dir = metrics_dir / track
        track_figures_dir = args.figures_dir / track
        track_model_dir = args.model_dir / track
        track_lgbm_dir = args.model_dir / "lightgbm_bagging" / track
        ...
        model_summary = train_and_evaluate_models(
            ...
            artifacts_metrics_dir=track_metrics_dir,
            ...
            lightgbm_model_dir=track_lgbm_dir,
        )
        track_summaries[track] = model_summary
        prevalence = model_summary.get("class_prevalence", {})
        track_prevalence_plot = track_figures_dir / f"attribution_split_distribution_{track}.png"
        _plot_split_prevalence(prevalence, track_prevalence_plot)
        _ensure_legacy_compat_copy(
            track_prevalence_plot,
            track_figures_dir / "fraud_split_distribution.png",
            compatibility_copy_map,
        )
        ...
        track_descriptors.append(
            {
                ...
                "metrics_path": _to_project_relative(track_metrics_dir / "model_metrics.csv"),
                "metrics_uncalibrated_path": _to_project_relative(track_metrics_dir / "model_metrics_uncalibrated.csv"),
                "metrics_calibrated_path": _to_project_relative(track_metrics_dir / "model_metrics_calibrated.csv"),
                ...
                if args.target_col == "is_attributed"
                else "legacy_is_fraud_deprecated",
            }
        )

        track_metrics_path = track_metrics_dir / "model_metrics.csv"
        if track_metrics_path.exists():
            track_df = pd.read_csv(track_metrics_path)
    ...
    if "production_safe" in tracks and len(stability_seeds) >= 2:
        ...
        for stab_seed in stability_seeds:
            stab_metrics_dir = metrics_dir / "production_safe" / "stability" / f"seed_{stab_seed}"
            stab_figures_dir = args.figures_dir / "production_safe" / "stability" / f"seed_{stab_seed}"
            stab_model_dir = args.model_dir / "production_safe" / "stability" / f"seed_{stab_seed}"
            stab_lgbm_dir = args.model_dir / "lightgbm_bagging" / "production_safe" / "stability" / f"seed_{stab_seed}"
    ...
    if combined_metric_frames:
        combined_df = pd.concat(combined_metric_frames, ignore_index=True)
        combined_df.to_csv(metrics_dir / "model_metrics.csv", index=False)
    ...
    split_plot_path = args.figures_dir / "attribution_split_distribution.png"
    _plot_split_prevalence(model_summary.get("class_prevalence", {}), split_plot_path)
    _ensure_legacy_compat_copy(split_plot_path, args.figures_dir / "fraud_split_distribution.png", compatibility_copy_map)
    save_json(
        {
        ...
        provenance_dir / "winner_style_run_metadata.json",
    )

    scored_pd = scored_df.select("veracity_score", args.target_col).toPandas()
    if not scored_pd.empty:
        ...
        plt.ylabel("Density")
        plt.tight_layout()
        plt.savefig(args.figures_dir / "veracity_score_distribution.png", dpi=150)
        plt.close()

    drift_metrics = validation_summary.get("drift_metrics", {})
    if drift_metrics:
        drift_rows = []
        ...
        plt.tight_layout()
        plt.savefig(args.figures_dir / "drift_metric_summary.png", dpi=150)
        plt.close()

    # Audit chain append + verification
    audit_log = audit_dir / "audit_log.jsonl"
    audit_records = [r.asDict(recursive=True) for r in curated_df.limit(20000).collect()]
    append_audit_batch(
        log_path=audit_log,
    ...
    save_json(verify_ok, audit_dir / "verification_report.json")

    tampered_log = tamper_log_copy(audit_log, audit_dir / "audit_log_tampered_demo.jsonl")
    verify_bad = verify_audit_log(tampered_log)
    save_json(verify_bad, audit_dir / "verification_report_tampered.json")

    overhead = measure_integrity_overhead(
        audit_records[: min(5000, len(audit_records))],
        loops=5,
        warmup=1,
    )
    save_json(overhead, metrics_dir / "integrity_overhead.json")

    # Plot fair benchmark comparison using median and IQR.
    ...
    fig.suptitle(
        f"Integrity Overhead (per-record overhead: {overhead['per_record_overhead_us']:.2f} µs)",
        fontsize=10,
    )
    plt.tight_layout()
    plt.savefig(args.figures_dir / "audit_overhead_throughput_latency.png", dpi=150)
    plt.close(fig)

    download_meta_path = provenance_dir / "dataset_download_metadata.json"
    ...
    run_metadata = build_run_metadata(
        ...
        extra={
            ...
            "curated_output_parquet": _to_project_relative(curated_path),
            "validation_report": _to_project_relative(metrics_dir / "validation_report.json"),
            "model_metrics": _to_project_relative(metrics_dir / "model_metrics.csv"),
            "winner_style_run_metadata": _to_project_relative(provenance_dir / "winner_style_run_metadata.json"),
            "approach_track": args.approach_track,
            "model_family": args.model_family,
            "row_cap": int(args.row_cap),
            "target_col": args.target_col,
    ...
    debug_outputs = _build_debug_outputs(
        ...
        legacy_semantics_warning=legacy_semantics_warning,
        stability_summary=stability_summary,
        compat_copy_map=compatibility_copy_map,
    )

    stream_metrics_path = metrics_dir / "streaming_metrics.csv"
    if stream_metrics_path.exists():
        try:
            stream_df = pd.read_csv(stream_metrics_path)
            if not stream_df.empty:
                batch_records = float(validation_summary.get("total_records", len(curated_pd)))
                batch_throughput = batch_records / etl_time_sec if etl_time_sec > 0 else 0.0
                comp_df = pd.DataFrame(
                    [
                        {
                            "pipeline": "batch",
                            "throughput_rps": batch_throughput,
                            "latency_sec": etl_time_sec,
                        },
                        {
                            "pipeline": "stream_avg",
                            "throughput_rps": float(stream_df["throughput_rps"].mean()),
                            "latency_sec": float(stream_df["latency_sec"].mean()),
                        },
                    ]
                )
                fig, axes = plt.subplots(1, 2, figsize=(11, 4))
                ...
                axes[1].set_ylabel("Seconds")
                plt.tight_layout()
                plt.savefig(args.figures_dir / "batch_vs_stream_comparison_summary.png", dpi=150)
                plt.close(fig)
        except Exception:
            pass
    ...
    summary = {
        ...
        "etl_time_sec": etl_time_sec,
        "training_time_sec": train_time_sec,
        "curated_parquet": _to_project_relative(curated_path),
        "validation_report": _to_project_relative(metrics_dir / "validation_report.json"),
        "model_metrics": _to_project_relative(metrics_dir / "model_metrics.csv"),
        "winner_style_run_metadata": _to_project_relative(provenance_dir / "winner_style_run_metadata.json"),
        "track_summaries": track_summaries,
        ...
        "shared_trust_controls": {
            "dq_validation_report": _to_project_relative(metrics_dir / "validation_report.json"),
            "audit_verification_report": _to_project_relative(audit_dir / "verification_report.json"),
            "integrity_overhead_report": _to_project_relative(metrics_dir / "integrity_overhead.json"),
            "streaming_metrics": _to_project_relative(metrics_dir / "streaming_metrics.csv"),
        },
        "audit_verification": verify_ok,
        "tamper_verification": verify_bad,
    }
    save_json(summary, metrics_dir / "batch_pipeline_summary.json")
